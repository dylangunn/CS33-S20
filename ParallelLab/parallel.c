...
double work_it_par(long *old, long *new, long *super, long *simple, long *fibonacci) {
  ...
  // GOOD
  int DIMsq = DIM*DIM;
  int iDIMsq,jDIM,iDIMsqjDIM,iDIMsqjDIMk,iuDIMsq,jvDIM,iDIMsqn,iDIMsqp,jDIMn,jDIMp,kn,kp;
  long we_need = we_need_the_func();
  long gimmie = gimmie_the_func();
  long templ;
#pragma omp parallel for private(j,k,iDIMsq,jDIM,iDIMsqjDIM,iDIMsqjDIMk,iDIMsqn,iDIMsqp,jDIMn,jDIMp,kn,kp,compute_it,templ) reduction(+:aggregate)
  for (i=1; i<DIMm; i++) {
      iDIMsq = i*DIMsq;
    ...
    for (j=1; j<DIMm; j++) {
      ...
      for (k=1; k<DIMm; k++) {
          ...
          kn=k-1;
          kp=k+1;
          compute_it = old[iDIMsqjDIMk] * we_need;
